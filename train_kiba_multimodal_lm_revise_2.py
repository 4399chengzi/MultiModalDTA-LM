# -*- coding: utf-8 -*-
# ======================================
# KIBA token-level 1D 冷启动训练脚本
#
# 说明：
#   1. 使用 LoadData_kiba_lm_1d_token_scaffold_seqsplit。
#   2. 使用新版 TokenLevelMultiModalDTA / MultiModalDTA_LM。
#   3. KIBA 无 3D 输入，因此模型自动走 1D-only 分支。
#   4. Dataset 使用 token bank + pair index，不展开 pair-level token，节省内存。
#   5. 训练与最终 best_model 统一通过 build_model 构建，避免参数不一致。
# ======================================
...
try:
    from util_load import LoadData_kiba_lm_1d_token_scaffold_seqsplit
except ImportError:
    from util import LoadData_kiba_lm_1d_token_scaffold_seqsplit
...
# 版本标记与路径
# ======================================
REV_TAG = "token_bidir_strict_split"
MODEL_MODULE_NAME = "model_token_crossmodal_lm_revise"
RESULT_ROOT_NAME = "result_revise"
DATASET_NAME = f"kiba_lm_1d_{REV_TAG}"

DATA_KIBA = r"../dataset/kiba"
OUT_DIR = f"../{RESULT_ROOT_NAME}/{DATASET_NAME}"

# 冷启动 token 缓存目录。
CACHE_DIR = "../dataset/kiba/processed_lm_1d_token_scaffold_seqsplit_602020"


# ======================================
# 训练超参数
# ======================================
# KIBA token 很大，Windows 下 num_workers 必须用 0，避免多进程复制 token bank。
BATCH_SIZE = 32
NUM_WORKERS = 0
PIN_MEMORY = False
...
# 论文正式结果建议 False，只在训练结束后评估一次 test。
# 如需复现旧训练日志风格，可改成 True。
...
# 严格 scaffold / sequence-disjoint 划分参数。
SPLIT_SEED = 2023
TRAIN_RATIO = 0.6
VAL_RATIO = 0.2
...
# ======================================
# 模型参数：训练模型与最终 best_model 必须完全一致
# ======================================
MODEL_D_MODEL = 256
MODEL_D_ATTN = 256
MODEL_N_HEADS = 4
...
# KIBA 没有 3D 输入，但模型初始化需要占位维度。
D3_LIG = 512
D3_POC = 512


# ======================================
# KIBA Dataset：token bank + pair index
# ======================================
class KibaTokenBankDataset(Data.Dataset):
    """
    使用唯一 ligand/protein token bank，通过 pair index 取样。
    不把 token 扩展到 pair-level，避免占用巨大内存。
    """
...
# collate
# ======================================
def kiba_token_collate(batch: list) -> Dict[str, Any]:
    """合并 KIBA token-level batch。"""
    ids = [b["id"] for b in batch]
    smiles = [b["smiles"] for b in batch]
    seqs = [b["seq"] for b in batch]

    pair_lig_idx = torch.as_tensor([b["pair_lig_idx"] for b in batch], dtype=torch.long)
    pair_pro_idx = torch.as_tensor([b["pair_pro_idx"] for b in batch], dtype=torch.long)
...
def forward_model(model: nn.Module, bdev: Dict[str, Any]) -> torch.Tensor:
    """
    KIBA 只有 1D token。
    不传 3D token，模型会自动走 has_seq 的 1D-only 路径。
    """
    return model(
...
def evaluate_kiba(
    ...
    if criterion is None:
        return ci, mse, rm2
    return ci, mse, rm2, loss_sum / max(1, n_batches)
...
if __name__ == "__main__":
    ...
    # ==================================
    # 加载 KIBA token-level 冷启动数据
    # ==================================
    kw_common = dict(
        out_1d=CACHE_DIR,
        logspace_trans=LOGSPACE_TRANS,
    ...
    mm_train = LoadData_kiba_lm_1d_token_scaffold_seqsplit(
        data_dir=DATA_KIBA, split="train", **kw_common
    )["train"]
    mm_val = LoadData_kiba_lm_1d_token_scaffold_seqsplit(
        data_dir=DATA_KIBA, split="val", **kw_common
    )["val"]
    mm_test = LoadData_kiba_lm_1d_token_scaffold_seqsplit(
        data_dir=DATA_KIBA, split="test", **kw_common
    )["test"]
    ...
    logging.info(
        f"[KIBA|Token dims] d_drug_lm={d_drug_lm}, d_prot_lm={d_prot_lm}"
    )
    logging.info(
        f"[KIBA|Token lengths] drug={mm_train['drug_lm_tokens_bank'].shape[1]}, "
        f"protein={mm_train['prot_lm_tokens_bank'].shape[1]}"
    )

    # ==================================
    # Dataset / DataLoader
    # ==================================
    train_ds = KibaTokenBankDataset(mm_train)
    val_ds = KibaTokenBankDataset(mm_val)
    test_ds = KibaTokenBankDataset(mm_test)
    ...
    logging.info(
        f"[KIBA|strict token] sizes: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}"
    )
    ...
    if SANITY_CHECK:
        ...
        print("drug_lm_tokens:", tuple(b["drug_lm_tokens"].shape),
              "| drug_lm_mask:", tuple(b["drug_lm_mask"].shape))
        print("prot_lm_tokens:", tuple(b["prot_lm_tokens"].shape),
              "| prot_lm_mask:", tuple(b["prot_lm_mask"].shape))
        print("y:", tuple(b["y"].shape))
        print("[OK] KIBA token-level 1D loaders ready.")
    ...
    with open(config_txt, "w", encoding="utf-8") as f:
        ...
        f.write(f"LOGSPACE_TRANS: {LOGSPACE_TRANS}\n")
    ...
    if not csv_file.exists():
        with open(csv_file, "w", newline="", encoding="utf-8") as f:
            header = [
                ...
                "train_CI", "train_MSE", "train_rm2",
                "val_CI", "val_MSE", "val_rm2",
                "lr",
            ]
            if EVAL_TEST_EACH_EPOCH:
                header.extend(["test_CI", "test_MSE", "test_rm2"])
    ...
    if RESUME_TRAINING and ckpt_path.exists():
        ...
        logging.info(f"[Resume|KIBA] from epoch: {start_epoch}")
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for last_bidx, b in enumerate(train_loader):
            ...
            if not torch.isfinite(loss):
                logging.warning(
                    f"[NaN|KIBA] non-finite loss at epoch={epoch}, batch={last_bidx}: {float(loss)}"
                )
                optimizer.zero_grad(set_to_none=True)
                continue
            ...
            if (last_bidx + 1) % ACCUM_STEPS == 0:
                ...
                if has_nan_params(model):
                    logging.error(
                        f"[NaN|KIBA] detected in params at epoch={epoch}, batch={last_bidx} before step"
                    )
                    optimizer.zero_grad(set_to_none=True)
                    break
                ...
                if has_nan_params(model):
                    logging.error(
                        f"[NaN|KIBA] detected in params at epoch={epoch}, batch={last_bidx} after step"
                    )
                    optimizer.zero_grad(set_to_none=True)
                    break
        ...
        # 尾批不足 ACCUM_STEPS 时，补一次 optimizer.step。
        ...
        # 评估 train / val。
        train_ci, train_mse, train_rm2, _ = evaluate_kiba(
            model, train_loader, DEVICE, criterion
        )
        val_ci, val_mse, val_rm2, val_loss = evaluate_kiba(
        ...
        writer.add_scalar("Metrics/train_rm2", train_rm2, epoch)
        writer.add_scalar("Metrics/val_CI", val_ci, epoch)
        writer.add_scalar("Metrics/val_MSE", val_mse, epoch)
        writer.add_scalar("Metrics/val_rm2", val_rm2, epoch)

        test_ci = test_mse = test_rm2 = None
        if EVAL_TEST_EACH_EPOCH:
            test_ci, test_mse, test_rm2 = evaluate_kiba(
            ...
            writer.add_scalar("Metrics/test_rm2", test_rm2, epoch)
        ...
        if EVAL_TEST_EACH_EPOCH:
            row.extend([test_ci, test_mse, test_rm2])
        ...
        if improved and torch.isfinite(torch.tensor(score_now)):
            ...
            epochs_no_improve += 1
            logging.info(
                f"[EARLY-STOP|KIBA] no improve for {epochs_no_improve} epoch(s); "
                f"best_score={best_score:.4f}"
            )
            if epochs_no_improve >= EARLY_STOP_PATIENCE:
                logging.info(
                    f"[EARLY-STOP|KIBA] patience={EARLY_STOP_PATIENCE} reached at epoch {epoch + 1}, stop training."
                )
        ...
        if EVAL_TEST_EACH_EPOCH:
            logging.info(
                f"[KIBA] Epoch {epoch + 1:04d} | "
                f"train_loss={train_loss:.6f} | val_loss={val_loss:.6f} | "
                f"train(CI={train_ci:.4f}, MSE={train_mse:.4f}, rm2={train_rm2:.4f}) | "
                f"val(CI={val_ci:.4f}, MSE={val_mse:.4f}, rm2={val_rm2:.4f}) | "
                f"test(CI={test_ci:.4f}, MSE={test_mse:.4f}, rm2={test_rm2:.4f}) | "
    ...
    with torch.no_grad():
        final_ci, final_mse, final_rm2 = evaluate_kiba(
            best_model, test_loader, DEVICE, criterion=None
        )

    logging.info(
        f"[FINAL|KIBA] test_CI={final_ci:.4f}, "
        f"test_MSE={final_mse:.4f}, test_rm2={final_rm2:.4f}"
    )

    with open(final_txt, "w", encoding="utf-8") as f:
        f.write("Final test was evaluated with the best model selected by validation MSE.\n")
