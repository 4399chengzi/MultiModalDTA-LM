...
DATASET_NAME = "atom3d_si30_mm_lm"                                 # 行：结果目录名（1D+3D）
...
class MultiModalDatasetLM(Data.Dataset):
    ...
    def __init__(self, pkg: Dict[str, Any]):
        ...
        # 2D 图在导入函数中已经去掉，这里保留占位字段，恒为 None，便于模型接口兼容
    ...
    for b in loader:
        ...
        with torch.cuda.amp.autocast(enabled=(USE_AMP and device.type == "cuda")):
            y_hat = model(
                ...
                g_lig=g_lig, g_prot=g_prot,   # 行：传入 None，模型内部会判 has_2d=False
...
if __name__ == "__main__":
    ...
    if SANITY_CHECK:
        ...
        print("drug_lm:", tuple(b['drug_lm'].shape),
              ...
              "| y:", tuple(b['y'].shape))
        print("g_lig:", b['g_lig'])       # 行：应为 None
        print("g_prot:", b['g_prot'])     # 行：应为 None
        print("[OK] 1D+3D LM loaders ready.")

    # —— 导入模型 —— #
    from model.model_multimodal_lm import MultiModalDTA_LM  # 行：多模态模型

    model = MultiModalDTA_LM(
        d_drug_lm=d_drug_lm, d_prot_lm=d_prot_lm,
        drug_node_dim=70, drug_edge_dim=6,    # 行：2D 模态虽未用，但构造函数需要
        prot_node_dim=33, prot_edge_dim=3,
        gcn_hidden=128, gcn_out=128,
        d3_lig=512, d3_poc=512,               # 行：与 Uni-Mol2 输出维度匹配
        d_model=256, d_attn=256, n_heads=4,
        add_interactions_3d=True
    ).to(DEVICE)

    # 优化器 / 损失 / 调度 / AMP
    criterion = nn.MSELoss(reduction='mean')   # 行：MSE 损失
    ...
    result_dir = result_root / "train_logs"    # 行：训练日志/权重目录
    ...
    final_txt  = result_dir / "final_test_metrics.txt"  # 行：最终测试结果
    run_dir.mkdir(parents=True, exist_ok=True)
    result_dir.mkdir(parents=True, exist_ok=True)
    writer = SummaryWriter(log_dir=str(run_dir))      # 行：TensorBoard writer

    # 断点恢复（这里只恢复 epoch 与模型/优化器参数，不恢复早停状态）
    start_epoch = 0
    if ckpt_path.exists():
    ...
    # ========== 训练主循环 ==========
    best_score = float('inf')                  # 行：记录最优 RMSE
    epochs_no_improve = 0                      # 行：早停累计未提升 epoch 数
    t0 = time.time()                           # 行：起始时间
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for last_bidx, b in enumerate(train_loader):
            drug_lm = b['drug_lm'].to(DEVICE, non_blocking=True)
            prot_lm = b['prot_lm'].to(DEVICE, non_blocking=True)
            g_lig, g_prot = b['g_lig'], b['g_prot']       # 行：None
            lig3d = b['lig3d'].to(DEVICE, non_blocking=True)
            poc3d = b['poc3d'].to(DEVICE, non_blocking=True)
            y_true = b['y'].to(DEVICE, non_blocking=True)
        ...
        # 评估（train/val 用同一 evaluate，val 用于早停）
        ...
        # —— 早停 + 最优权重 —— #
        score_now = val_rmse if VAL_AS_BEST else test_rmse    # 行：当前用来比较的 RMSE
        improved = (best_score - score_now) > EARLY_STOP_MIN_DELTA  # 行：是否有“显著提升”

        if improved and torch.isfinite(torch.tensor(score_now)):
            best_score = score_now
            epochs_no_improve = 0                              # 行：重置早停计数
            ...
            epochs_no_improve += 1                             # 行：未提升 epoch+1
            logging.info(
