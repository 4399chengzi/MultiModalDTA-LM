# ======================================
# KIBA 1D LM 版训练脚本（带早停）
# 指标：CI, MSE, rm2   （不再计算 AUPR）
...
import util
from util import LoadData_kiba_lm_1d           # 行：KIBA 1D LM 数据加载函数
...
# ========== 可配置区域 ==========
DATA_KIBA    = r"../dataset/kiba"              # 行：KIBA 原始数据目录（含 ligands_can.txt / proteins.txt / Y）
DATASET_NAME = "kiba_lm_1d"                    # 行：结果任务名，用于存放日志/模型
OUT_DIR      = f"../result/{DATASET_NAME}"     # 行：结果根目录

BATCH_SIZE   = 64                             # 行：KIBA 样本多，适当加大 batch（根据显存可再调）
NUM_WORKERS  = 4                               # 行：DataLoader 的 worker 数（Windows 可改 0/2）
PIN_MEMORY   = bool(torch.cuda.is_available()) # 行：GPU 时开启 pin_memory 加速拷贝

EPOCHS         = 600                           # 行：KIBA 比较大，设置 400 个 epoch（配合早停）
...
# ========== KIBA 1D LM Dataset ==========
class KibaLMDataset(Data.Dataset):
    """封装 KIBA 1D LM 特征（不包含 2D/3D）。"""
    def __init__(self, pkg: Dict[str, Any]):
        self.ids     = pkg["ids"]        # 行：np.array(object)，样本 ID
        self.y       = pkg["y"]          # 行：np.float32，标签（KIBA 分数）
...
def kiba_lm_collate(batch: list) -> Dict[str, Any]:
...
# ========== KIBA 评估函数：CI / MSE / rm2（不算 AUPR）==========
@torch.no_grad()
def evaluate_kiba(model: torch.nn.Module,
...
if __name__ == "__main__":
    ...
    # —— 加载 KIBA 1D LM 数据 —— #
    kw_common = dict(
        out_1d="../dataset/kiba/processed_lm_1d",  # 行：缓存目录，要和 util 中保持一致
        logspace_trans=False,                      # 行：KIBA 一般不做 log 转换
    ...
    mm_train = LoadData_kiba_lm_1d(data_dir=DATA_KIBA, split="train", **kw_common)["train"]  # 行：训练集
    mm_val   = LoadData_kiba_lm_1d(data_dir=DATA_KIBA, split="val",   **kw_common)["val"]    # 行：验证集
    mm_test  = LoadData_kiba_lm_1d(data_dir=DATA_KIBA, split="test",  **kw_common)["test"]   # 行：测试集

    # LM 维度
    d_drug_lm = mm_train["drug_lm"].shape[1]      # 行：药物 LM 特征维度
    d_prot_lm = mm_train["prot_lm"].shape[1]      # 行：蛋白 LM 特征维度
    logging.info(f"[KIBA|LM] drug_lm_dim={d_drug_lm}, prot_lm_dim={d_prot_lm}")

    # —— Dataset / DataLoader —— #
    train_ds = KibaLMDataset(mm_train)            # 行：构造训练集 Dataset
    val_ds   = KibaLMDataset(mm_val)              # 行：构造验证集 Dataset
    test_ds  = KibaLMDataset(mm_test)             # 行：构造测试集 Dataset

    train_loader = Data.DataLoader(               # 行：训练 DataLoader
        train_ds, batch_size=BATCH_SIZE, shuffle=True,
        num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY,
        collate_fn=kiba_lm_collate
    )
    ...
    logging.info(f"[KIBA] sizes: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")
    ...
    if SANITY_CHECK:
        ...
        print("drug_lm:", tuple(b["drug_lm"].shape),
              ...
              "| y:", tuple(b["y"].shape))
        print("[OK] KIBA LM loaders ready.")
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for last_bidx, b in enumerate(train_loader):
            ...
            if not torch.isfinite(loss):                          # 行：NaN/Inf 检查
                logging.warning(f"[NaN|KIBA] loss at epoch={epoch}, batch={last_bidx}: {float(loss)}")
        ...
        if score_now + EARLY_STOP_MIN_DELTA < best_score and torch.isfinite(torch.tensor(score_now)):
            ...
            logging.info(
                f"[EARLY-STOP|KIBA] epoch={epoch+1}: no improvement on "
            ...
            if epochs_no_improve >= EARLY_STOP_PATIENCE:           # 行：达到耐心上限，停止训练
                logging.info(
                    f"[EARLY-STOP|KIBA] Stop training at epoch {epoch+1}, "
    ...
    with torch.no_grad():
        final_ci, final_mse, final_rm2 = evaluate_kiba(        # 行：只算 CI/MSE/rm2
            best_model, test_loader, DEVICE, criterion=None
        )

    logging.info(
        f"[FINAL|KIBA] test_CI={final_ci:.4f}, test_MSE={final_mse:.4f}, "
