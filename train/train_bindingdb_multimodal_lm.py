# ======================================
# BindingDB 1D LM 版训练脚本（带早停）
# 指标：CI, MSE, rm2   （不计算 AUPR）
...
import util
from util import LoadData_bindingdb_lm_1d      # 行：BindingDB 1D LM 数据加载函数（你在 util 里实现的那个）
...
# ========== 可配置区域 ==========
DATA_BINDINGDB = r"../dataset/bindingdb"        # 行：BindingDB 原始数据目录（含 bindingdb_train.csv / bindingdb_test.csv）
DATASET_NAME  = "bindingdb_lm_1d"               # 行：结果任务名，用于存放日志/模型
OUT_DIR       = f"../result/{DATASET_NAME}"     # 行：结果根目录

BATCH_SIZE   = 64                               # 行：BindingDB 样本较多，batch 可适当大一点（可按显存调）
...
VAL_AS_BEST    = True                           # 行：以 val_MSE 作为“最优模型”标准

# ---- 早停相关参数 ----
EARLY_STOP_PATIENCE  = 40      # 行：若连续 40 个 epoch val_MSE 无提升，则提前停止
EARLY_STOP_MIN_DELTA = 0.0     # 行：认为“有提升”的最小幅度（新 MSE 至少比 best 小 0.0）

# ========== BindingDB 1D LM Dataset ==========
class BindingDBLMDataset(Data.Dataset):
    """封装 BindingDB 1D LM 特征（不包含 2D/3D）。"""
    def __init__(self, pkg: Dict[str, Any]):
        self.ids     = pkg["ids"]        # 行：np.array(object)，样本 ID（如 tr_0 / te_0）
        self.y       = pkg["y"]          # 行：np.float32，标签（affinity / pKd 等）
...
def bindingdb_lm_collate(batch: list) -> Dict[str, Any]:
...
# ========== BindingDB 评估函数：CI / MSE / rm2（不算 AUPR）==========
@torch.no_grad()
def evaluate_bindingdb(model: torch.nn.Module,
    ...
    for b in loader:
        ...
        with torch.cuda.amp.autocast(enabled=(USE_AMP and device.type == "cuda")):
            # 行：这里只用 1D LM，不传 2D/3D 图
            y_hat = model(drug_lm=drug_lm, prot_lm=prot_lm)    # 行：前向得到预测
            if criterion is not None:
                loss_val = criterion(y_hat, y_true)            # 行：计算 MSE
    ...
    # 转 numpy 计算指标
    ci   = util.get_cindex(obs, pred)          # 行：CI 指标
    mse  = util.get_MSE(obs, pred)             # 行：MSE
    rm2  = util.get_rm2(obs, pred)             # 行：rm2

    if criterion is None:
        return ci, mse, rm2                    # 行：无 loss 情况
    return ci, mse, rm2, (loss_sum / max(1, n_batches))   # 行：返回平均 loss
...
if __name__ == "__main__":
    ...
    # —— 加载 BindingDB 1D LM 数据 —— #
    kw_common = dict(
        out_1d="../dataset/bindingdb/processed_lm_1d",  # 行：缓存目录，要和 util 中保持一致
    ...
    mm_train = LoadData_bindingdb_lm_1d(
        data_dir=DATA_BINDINGDB, split="train", **kw_common
    )["train"]                                         # 行：训练集
    mm_val   = LoadData_bindingdb_lm_1d(
        data_dir=DATA_BINDINGDB, split="val", **kw_common
    )["val"]                                           # 行：验证集
    mm_test  = LoadData_bindingdb_lm_1d(
        data_dir=DATA_BINDINGDB, split="test", **kw_common
    )["test"]                                          # 行：测试集

    # LM 维度
    d_drug_lm = mm_train["drug_lm"].shape[1]           # 行：药物 LM 特征维度
    d_prot_lm = mm_train["prot_lm"].shape[1]           # 行：蛋白 LM 特征维度
    logging.info(f"[BindingDB|LM] drug_lm_dim={d_drug_lm}, prot_lm_dim={d_prot_lm}")

    # —— Dataset / DataLoader —— #
    train_ds = BindingDBLMDataset(mm_train)            # 行：构造训练集 Dataset
    val_ds   = BindingDBLMDataset(mm_val)              # 行：构造验证集 Dataset
    test_ds  = BindingDBLMDataset(mm_test)             # 行：构造测试集 Dataset

    train_loader = Data.DataLoader(                    # 行：训练 DataLoader
        train_ds, batch_size=BATCH_SIZE, shuffle=True,
        num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY,
        collate_fn=bindingdb_lm_collate
    )
    ...
    logging.info(f"[BindingDB] sizes: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")
    ...
    if SANITY_CHECK:
        ...
        print("drug_lm:", tuple(b["drug_lm"].shape),
              ...
              "| y:", tuple(b["y"].shape))
        print("[OK] BindingDB LM loaders ready.")
    ...
    if ckpt_path.exists():
        ...
        logging.info(f"[Resume|BindingDB] from epoch: {start_epoch}")
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for last_bidx, b in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=(USE_AMP and DEVICE.type == "cuda")):
                ...
                loss = criterion(y_hat, y_true)                   # 行：计算 MSE

            if not torch.isfinite(loss):                          # 行：NaN/Inf 检查
                logging.warning(f"[NaN|BindingDB] loss at epoch={epoch}, batch={last_bidx}: {float(loss)}")
        ...
        # —— 评估：train/val/test（只算 CI/MSE/rm2）—— #
        train_ci, train_mse, train_rm2, _ = evaluate_bindingdb(
            model, train_loader, DEVICE, criterion
        )
        val_ci,   val_mse,   val_rm2,   val_loss = evaluate_bindingdb(
            model, val_loader,   DEVICE, criterion
        )
        test_ci,  test_mse,  test_rm2            = evaluate_bindingdb(
        ...
        score_now = val_mse if VAL_AS_BEST else test_mse          # 行：当前早停指标
        ...
        if score_now + EARLY_STOP_MIN_DELTA < best_score and torch.isfinite(torch.tensor(score_now)):
            ...
            logging.info(
                f"[BEST|BindingDB] epoch={epoch+1}, "
            ...
            logging.info(
                f"[EARLY-STOP|BindingDB] epoch={epoch+1}: no improvement on "
            ...
            if epochs_no_improve >= EARLY_STOP_PATIENCE:           # 行：达到耐心上限，停止训练
                logging.info(
                    f"[EARLY-STOP|BindingDB] Stop training at epoch {epoch+1}, "
                    f"no improvement for {EARLY_STOP_PATIENCE} epochs."
                )
                break

        logging.info(
            f"[BindingDB] Epoch {epoch+1:04d} | "
    ...
    with torch.no_grad():
        final_ci, final_mse, final_rm2 = evaluate_bindingdb(    # 行：只算 CI/MSE/rm2
            best_model, test_loader, DEVICE, criterion=None
        )

    logging.info(
        f"[FINAL|BindingDB] test_CI={final_ci:.4f}, test_MSE={final_mse:.4f}, "
        f"test_rm2={final_rm2:.4f}"
    )
