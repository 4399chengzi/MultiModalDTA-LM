...
# （数值稳定版：关闭 AMP、降低 LR、加 NaN 保护，1D+3D，无 2D 图）
# ======================================
...
DATASET_NAME = "atom3d_si60_mm_lm"                                       # 行：任务名（区分输出目录）
...
class MultiModalDatasetLM(Data.Dataset):
    ...
    def __init__(self, pkg: Dict[str, Any]):
        ...
        self.has_2d  = (self.g_lig is not None) and (self.g_prot is not None)
...
if __name__ == "__main__":
    ...
    logging.info(f"[si-60] ratios(raw): "
    ...
    logging.info(f"[si-60|LM] final sizes: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}")
    ...
    # —— 导入模型（需与你的 MultiModalDTA_LM 兼容 g_lig/g_prot=None） —— #
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        for last_bidx, b in enumerate(train_loader):
            ...
            if (last_bidx + 1) % ACCUM_STEPS == 0:
                ...
                if has_nan_params(model):
                    logging.error(f"[NaN] detected in params at epoch={epoch}, batch={last_bidx} (before step)")
                    optimizer.zero_grad(set_to_none=True)
                    break
                ...
                if has_nan_params(model):
                    logging.error(f"[NaN] detected in params at epoch={epoch}, batch={last_bidx} (after step)")
                    optimizer.zero_grad(set_to_none=True)
                    break
        ...
        if torch.isfinite(torch.tensor(score_now)) and (improved or best_score == float('inf')):
            ...
            epochs_no_improve += 1
            logging.info(f"[ES] no improvement for {epochs_no_improve} epoch(s) "
