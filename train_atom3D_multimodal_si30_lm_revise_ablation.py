# ======================================
# 多模态 ATOM3D(SI-30) 批量消融训练脚本（LM 版）
# 模态：当前脚本实际使用 1D(LM) + 3D(Uni-Mol2)，2D 图模态关闭
# 功能：
#   1) 一次性跑完当前 1D+3D 场景相关的全部消融实验
#   2) 每个消融模式单独建目录、单独保存 best model / checkpoint / csv / final metrics
#   3) 最终把所有消融结果汇总到一张总表
# 依赖：
...
import util
from util import LoadData_atom3d_si30_multimodal_lm
...
REV_TAG = "revise_ablation"
MODEL_MODULE_NAME = "model_multimodal_lm_revise_ablation"
RESULT_ROOT_NAME = "result_revise"
DATASET_NAME = f"atom3d_si30_mm_lm_{REV_TAG}"

# ========== 当前脚本可运行的全部相关消融 ==========
# 注意：
# 1) 当前 SI-30 训练脚本没有 2D 图输入，因此不跑 2D_only / 2d_3d_* / 1d_2d_3d_*。
...
class MultiModalDatasetLM(Data.Dataset):
    ...
    def __init__(self, pkg: Dict[str, Any]):
        ...
        self.seq     = pkg['seq']
        self.drug_lm = pkg['drug_lm']
        self.prot_lm = pkg['prot_lm']
        self.g_lig   = pkg.get('g_lig', None)
        self.g_prot  = pkg.get('g_prot', None)
        self.lig3d   = pkg['lig_3d']
        self.poc3d   = pkg['poc_3d']
...
def run_one_ablation(
    ...
    训练一个消融模式：
      ...
      3) 最终把结果追加到总汇总表
    """
    util.seed_torch()  # 每个消融模式都用同一随机种子，保证更公平
...
if __name__ == "__main__":
    ...
    # —— 加载数据（只加载一次，后面所有消融共用） —— #
    kw_common = dict(
        out_mm="../dataset/ATOM3D/processed_mm_si30_lm",
    ...
    logging.info(
        f"[si-30|LM] final sizes: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}"
    )
