# -*- coding: utf-8 -*-
# ======================================
# ATOM3D SI-60 token-level 多模态训练脚本
#
# 作用：
#   1. 使用 token-level LoadData_atom3d_si60_multimodal_lm_token。
...
try:
    from util_load import LoadData_atom3d_si60_multimodal_lm_token
except ImportError:
    from util import LoadData_atom3d_si60_multimodal_lm_token
...
DATASET_NAME = f"atom3d_si60_mm_lm_{REV_TAG}"

DATA_DB_SI60 = r"../dataset/ATOM3D/split-by-sequence-identity-60/data"
...
CACHE_DIR = "../dataset/ATOM3D/processed_mm_si60_lm_token"
...
# 统计 si-60 拆分
# ======================================
def count_si60_splits(root_base: str) -> Tuple[Dict[str, int], Dict[str, float], int]:
...
if __name__ == "__main__":
    ...
    mm_train = LoadData_atom3d_si60_multimodal_lm_token(
        root_base=DATA_DB_SI60, split="train", **kw_common
    )["train"]
    mm_val = LoadData_atom3d_si60_multimodal_lm_token(
        root_base=DATA_DB_SI60, split="val", **kw_common
    )["val"]
    mm_test = LoadData_atom3d_si60_multimodal_lm_token(
    ...
    logging.info(
        f"[si-60|token] final sizes: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}"
    )
    ...
    with open(config_txt, "w", encoding="utf-8") as f:
        ...
        f.write(f"DATA_DB_SI60: {DATA_DB_SI60}\n")
